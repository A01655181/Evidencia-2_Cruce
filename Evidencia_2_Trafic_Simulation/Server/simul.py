...
class Model (mesa.Model):
    ...
    def step(self):
        ...
        response = {}
        response["cars"] = [
                {"pos":[int(agent.pos[0]), int(agent.pos[1])], "name":agent.unique_id}
                for agent in self.schedule_car.agents]
        response["tf"] = [
            {"pos": [int(agent.pos[0]), int(agent.pos[1])], "status":agent.status}
            for agent in self.schedule_tf.agents]
        # print(response)
        return response
        # res = json.JSONEncoder().encode(response)
